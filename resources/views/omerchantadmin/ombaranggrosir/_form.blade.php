...
<div class="form-group {!! $errors->has('varian_id') ? 'has-error' : '' !!}">
    ...
    <div class="col-md-10">
        @if(empty($data))
            {!! Form::select('varian_id',[''=>''], null,['class'=>'js-selectize', 'placeholder'=>'Pilih Barang','disabled'=>isset($om_barang_varian->barang_id) ? 'disabled' : null]) !!}
        @else
...
$(function(){
    $('select[name="barang_id"]').change(function(){
        {{ $link = url('/omerchantadmin/barangvarian/data/') }}
        $.ajax({
